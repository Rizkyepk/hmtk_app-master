...
class BankMateri extends StatelessWidget {
  BankMateri({super.key});

  List<Map<String, dynamic>>? data;

  Future<List<Map<String, dynamic>>> getMaterialBanks() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data["success"]) {
          return List<Map<String, dynamic>>.from(data["bank_materials"]);
        } else {
          throw data["message"];
        }
      } else {
  ...
  Color getColor(int index) {
    return index % 2 == 0
        ? const Color.fromARGB(255, 96, 221, 100)
        : Colors.green;
  }
      ...
      body: FutureBuilder(
        ...
        builder: (BuildContext context,
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            List<ItemBank> buildLevelList(int level) {
              List<Map<String, dynamic>> levelData =
                  data!.where((item) => item['level'] == level).toList();
              return List<ItemBank>.generate(levelData.length, (index) {
                var item = levelData[index];
                return ItemBank(
                  txt: item['subject'],
            ...
            return ListView(
              children: [
                buildItemList('Tingkat 1', buildLevelList(1)),
                buildItemList('Tingkat 2', buildLevelList(2)),
                buildItemList('Tingkat 3', buildLevelList(3)),
                buildItemList('Tingkat 4', buildLevelList(4)),
              ],
            );
          }
        },
